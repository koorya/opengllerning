...
Mesh::Mesh(aiMesh * mesh, const aiScene * scene, std::vector<Texture> textures, std::vector <glm::mat4> instance_mat4, bool print_vert){
	...
	if(print_vert){
		for(int i=0; i<indices.size(); i++){
			std::clog<<i<<" index point "<<mesh->mVertices[indices[i]].x<<" "<<mesh->mVertices[indices[i]].y<<" "
			<<mesh->mVertices[indices[i]].z<<" "<<std::endl;
		}
	}
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.size()*sizeof(unsigned int), indices.data(), GL_STATIC_DRAW);
